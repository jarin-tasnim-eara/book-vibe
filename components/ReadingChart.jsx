...
export default function ReadingChart(){
  ...
  const step = maxPages /4;
  const ySteps = [0,step,step*2,step*3,maxPages,];
    ...
    <div className="bg-gray-50 rounded-xl p-8 overflow-x-auto">
      <div className="flex">
        ...
        style={{ height: CHART_HEIGHT }}>
          {[...ySteps].reverse().map((value) =>(
             <span key={value}> {Math.round(value)}</span>
          ))}
        </div>

        <div className="relative flex-1" 
        style={{height: CHART_HEIGHT, minWidth: books.length *(BAR_WIDTH +30), }}>

          <div className="absolute inset-0 flex flex-col justify-between">
            {ySteps.map((value) =>(
              <div key={value} className="border-t border-dashed border-gray-200 w-full"/>
            ))}
          </div>
          ...
          <div className="relative flex justify-around items-end h-full">
            {books.map((book, index) =>{
              ...
              const height =(book.pages / maxPages) * CHART_HEIGHT;
              ...
              return (
                <div key={book.id} className="flex flex-col items-center" style={{ width: BAR_WIDTH }}>
                  ...
                  <svg width={BAR_WIDTH} height={height} viewBox={`0 0 ${BAR_WIDTH} ${height}`}>
                    <path
                      d={`M0,${height}
                          Q${BAR_WIDTH *0.12},${height *0.35}
                          ${BAR_WIDTH /2},0
                          Q${BAR_WIDTH *0.88},${height *0.35}
                          ${BAR_WIDTH},${height}
                          Z`}
                      fill={color}
                    />
      ...
      <div className="flex justify-around mt-3 pl-10"
        style={{ minWidth: books.length *(BAR_WIDTH + 30), }}>
        {books.map((book) =>(
          <span key={book.id} className="text-xs text-gray-500 text-center" style={{width: BAR_WIDTH }} >{book.title}</span>
        ))}
      </div>
    </div>
  );
